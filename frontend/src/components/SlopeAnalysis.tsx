...
export function SlopeAnalysis() {
  ...
  const emergencyContacts = [
    { name: "BASARNAS Wonosobo", phone: "0286-123456", type: "emergency" },
    { name: "BPBD Wonosobo", phone: "0286-789012", type: "disaster" },
    {
    ...
    {
      name: "Grup Instagram @WonosoboAlert",
      instagram: "@wonosoboAlert",
      type: "social",
    },
  ];
  ...
  useEffect(() => {
    const sessionKey = "logged_slope_session_" + new Date().toISOString().split('T')[0];
    if (!sessionStorage.getItem(sessionKey)) {
      logActivity("Sesi GIS Lereng");
  ...
  const handleSendToBasarnas = async () => {
    ...
    setTimeout(() => {
      toast.success("Alert berhasil dikirim ke BASARNAS Wonosobo");
      // Log the alert
      const logEntry = {
...
Via EcoScope Wonosobo`;
  ...
  const generateReport = () => {
    ...
    const reportContent = `LAPORAN ANALISIS LERENG - ECOSCOPE WONOSOBO
...
Generated by EcoScope Wonosobo
