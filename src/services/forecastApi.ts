...
export const forecastCommodityPrice = async (
  ...
  try {
    const url = `${BACKEND_API}/forecast/commodity/${encodeURIComponent(commodityName)}?days_forward=${daysForward}&days_back=${daysBack}&use_synthetic=${useSynthetic}`;
...
export const quickPricePrediction = async (
  ...
  try {
    const url = targetDate
      ? `${BACKEND_API}/forecast/quick-predict/${encodeURIComponent(commodityName)}?target_date=${targetDate}`
      : `${BACKEND_API}/forecast/quick-predict/${encodeURIComponent(commodityName)}`;
