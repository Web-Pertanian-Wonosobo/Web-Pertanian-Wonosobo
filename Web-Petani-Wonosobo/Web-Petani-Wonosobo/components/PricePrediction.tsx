...
export function PricePrediction() {
  ...
  // 📝 Fungsi baru untuk download laporan prediksi dalam format PDF
  const downloadPredictionReport = () => {
    ...
    doc.setFontSize(14);
    doc.text("ECOSCOPE BANYUMAS", 10, 28);
    ...
    const addSection = (title: string, content: string) => {
      ...
      y += 6;
      doc.line(10, y, 200, y); // Garis pemisah
      y += 8;
    ...
    addSection("Prediksi Harga (2 Minggu Ke Depan)", predictionText);
    ...
    addSection("Simulasi Pendapatan", simulationText);
    ...
    toast.success("Laporan prediksi berhasil didownload dalam bentuk PDF");
  };

  // 📝 Fungsi baru untuk download simulasi dalam format PDF
  const downloadSimulationReport = () => {
    ...
    doc.setFontSize(22);
    doc.text("LAPORAN SIMULASI PENDAPATAN", 10, 20);
    doc.setFontSize(14);
    doc.text("ECOSCOPE BANYUMAS", 10, 28);
    ...
    addSection(
      ...
      `Komoditas\t\t: ${currentCommodity.name}\n` +
        `Jumlah Panen\t: ${simulationData.harvestAmount} kg\n` +
        `Tanggal Panen\t: ${
          simulationData.harvestDate || "Tidak ditentukan"
        }\n` +
        `Harga Saat Ini\t: Rp ${currentCommodity.currentPrice.toLocaleString()}\n` +
        `Estimasi Harga\t: Rp ${simulationData.estimatedPrice.toLocaleString()}`
    );
    ...
    doc.save(
      `simulasi-pendapatan-${selectedCommodity}-${
        new Date().toISOString().split("T")[0]
      }.pdf`
    );

    toast.success("Laporan simulasi berhasil didownload dalam bentuk PDF");
  };
